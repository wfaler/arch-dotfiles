...
# gpu_info is reused later to generate ~/.config/hypr/hardware.lua.
...
# Generate ~/.config/hypr/hardware.lua based on detected GPU.
# hyprland.lua require()s this; we always write the file (even if empty body)
# so the require doesn't fail.
hardware_lua="$HOME/.config/hypr/hardware.lua"
mkdir -p "$(dirname "$hardware_lua")"
{
